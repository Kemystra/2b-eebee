...
void NukeBot::nuke() {
    selfLog("NukeBot is nuking the world!");
    vector<GenericRobot* > allRobots = environment->getAllAvailableRobots();
    for (GenericRobot* robot : allRobots) {
        if (robot != this && robot->getLivingState() == Alive) {
            Vector2D targetRelPos = robot->getPosition() - this->getPosition();
            fire(targetRelPos.x, targetRelPos.y);
        }
    }
}

void NukeBot::thinkAndExecute() {
    if (nukeCount > 0) {
